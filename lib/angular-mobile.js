			...
			if (nodeName != "#comment" && nodeName != "SCRIPT") {
				...
				if (attrs && attrs.length > 0) {
					for (var j = 0; j < attrs.length; j++) {
						...
						} else if (node_n == "am-class") {
							var originClass = nodes[i].getAttribute('class');
							var newClass = this._invoke_expression(node_v, nodes[i]);
							nodes[i].setAttribute('class', nodes[i].className + " " + newClass);
						} else {
							if (node_v.indexOf('{{') >= 0) {
