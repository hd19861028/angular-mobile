/**
 * 作者：胡锦柢
 * 用途：适用于移动端的angular迷你版，压缩大小：4.65k，gzip大小：1.49k
 * 依赖：纯原生js，无任何依赖
 ...
 * */
function angular_mobile() {
	window.scope = {};
	window.scope.elements = [];
	window.scope.events = [];

	this._get_value = function(attr, e) {
	...
	this._invoke_expression = function(exp, e) {
		var content;
		if (exp.indexOf('?') >= 0) {
			...
			if (sanyuan[0].indexOf('==') >= 0 && result == null) {
				...
				if (this._get_value(temp[0]).toString() == temp[1]) result = sanyuan_T;
	...
	this._bind_repeat = function() {
		for (var i = 0; i < document.all.length; i++) {
			...
			if (attr) {
				var dataset = this._get_value(attr);
				...
				if (dataset && dataset.length > 0) {
					for (var j = 0; j < dataset.length; j++) {
						...
						while (start >= 0) {
							...
							else value = row[tempexp] ? row[tempexp] : '';
		...
		"set": function(name, obj) {
			...
			var timer = setInterval(function() {
				if (document.body) {
					...
					o._bind_src();
					o._add_event(window, 'onscroll', function() {
						o._bind_src();
					})
					clearInterval(timer)
				}
